...
function AudioPlayer({ audioUrl, title }) {
  ...
  // مشاركة المقطع الصوتي==============================
  const shareAudio = () => {
    if (navigator.share) {
      // إذا دعم المتصفح Web Share API
      navigator
        .share({
          title: title,
          text: `استمع لهذا الدرس: ${title}`,
          url: audioUrl,
        })
        .catch((error) => console.log("خطأ بالمشاركة:", error));
    } else {
      // إذا لم يدعم المتصفح، نسخ الرابط إلى الحافظة
      navigator.clipboard.writeText(audioUrl).then(() => {
        alert("تم نسخ رابط الصوت إلى الحافظة!");
      });
    }
  };
// ===========================================
  ...
  return (
    <div style={{ marginTop: 10, textAlign: "center" }}>
      {audioUrl.includes("youtu") ? (
        <div style={{ position: "relative", paddingBottom: "56.25%", height: 0, marginTop: "10px" }}>
          <iframe
...
>
  📤 مشاركة
          ...
          <div style={{ margin: "10px 0" }}>
            <input type="range" min="0" max={duration} value={currentTime} onChange={handleSeek} style={{ width: "70%" }} />
...
export default function HadithLessons() {
  ...
  return (
    <div style={{ maxWidth: 800, margin: "20px auto", padding: 20 }}>
      <h1 style={{ textAlign: "center", color: "green", fontWeight: "bold" }}>متفرقات</h1>
      ...
      {lessons.map(({ id, title, description, author, audioUrl, transcript }) => (
        ...
        >
          ...
          {author && <p style={{ textAlign: "center", color: "green", fontWeight: "bold" }}>{author}</p>}
